...
const JambSingle = () => {
  ...
  const { data, loading, error } = useFetch(
    currentSession ? `/jamb-exam/${id}/${currentSession._id}` : null
  );
  ...
  const handleDeleteQuestion = async (questionId) => {
    ...
    try {
      ...
      const response = await fetch(
        `${apiUrl}/api/jamb-questions/${questionId}`,
        {
          method: "DELETE",
          headers: {
  ...
  useEffect(() => {
    const fetchQuestions = async () => {
      try {
        ...
        const response = await fetch(`${apiUrl}/api/jamb-questions/${id}`, {
          method: "GET",
  ...
  const submitQuestion = async () => {
    try {
      ...
      const response = await fetch(
        `${apiUrl}/api/jamb-questions/${sessionId}`,
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
